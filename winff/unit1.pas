...
//    mitPlaySoundonFinish: TMenuItem;
    ...
    procedure AddFilesToList(InitialDir: string; Files: TStrings);
...
begin
  ...
  presetspath := ExtractFilePath(Application.ExeName);
...
procedure TfrmMain.AddFilesToList(InitialDir: string; Files: TStrings);
var
  u: string;
  s: string;
  i: integer;
begin
  setconfigvalue('general/addfilesfolder', InitialDir);
...
begin
   ...
   if dlgOpenFile.Execute then
      begin
        AddFilesToList(dlgOpenFile.InitialDir, dlgOpenFile.Files);
      end;
   filelist.hint := 'Number of jobs in queue: ' + IntTostr(filelist.count);
   if multipresets then pnlAllow.Visible := False;
end;
